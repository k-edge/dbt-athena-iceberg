...
{{
  config(
    ...
    incremental_predicates=[
      "DBT_INTERNAL_DEST.last_updated_at >= date_add('day', -" ~ merge_lookback_days ~ ", current_timestamp)"
    ]
  )
}}
...
-- Optional row limit for development/testing runs.
-- Override with `--vars '{row_limit: null}'` (or a different number).
